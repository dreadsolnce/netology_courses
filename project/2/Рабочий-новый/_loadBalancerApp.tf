...
resource "yandex_alb_target_group" "target-group" {
  ...
  target {
    ...
    subnet_id   = each.key == "front" ? yandex_vpc_subnet.subnet["public"].id : (each.key == "back" ? yandex_vpc_subnet.subnet["private"].id : "unknown")
  }
}
...
resource "yandex_alb_load_balancer" "create-alb" {
  ...
  allocation_policy {
    location {
      zone_id   = var.subnets.public.zone
      subnet_id = yandex_vpc_subnet.subnet["public"].id
    }
    location {
      zone_id   = var.subnets.private.zone
