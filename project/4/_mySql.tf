...
resource "yandex_mdb_mysql_cluster" "mysql-cluster" {
  ...
  dynamic "host" {
    # for_each = yandex_vpc_subnet.ssubnet-private
    for_each = { for k in local.flattened_subnets: k.name => k if strcontains(k.name, "app-private")  }
    content {
      zone      = host.value.zone
      # subnet_id = host.value.id
      subnet_id = yandex_vpc_subnet.subnet[host.key].id
    }
  }
