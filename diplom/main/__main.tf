...
data "template_file" "cloudinit-bastion" {
  ...
  vars = {
    ...
    file_content              = templatefile("${path.module}/proxy.tftpl", {
        ...
        file_privkey_proxy    = fileexists(var.file_privkey) ? filebase64(var.file_privkey) : ""
        file_fullchain_proxy  = fileexists(var.file_fullchain) ? filebase64(var.file_fullchain) : ""
        file_chain_proxy      = fileexists(var.file_chain) ? filebase64(var.file_chain) : ""
    })
    ...
    file_privkey_cloud              = fileexists(var.file_privkey) ? filebase64(var.file_privkey) : ""
    file_fullchain_cloud            = fileexists(var.file_fullchain) ? filebase64(var.file_fullchain) : ""
    file_chain_cloud                = fileexists(var.file_chain) ? filebase64(var.file_chain) : ""
  }
}
